...
class room(baseObject):
    ...
    def getRoomStats(self):
        ...
        # Min/Max Room Prices by Type
        sql_min_max_prices = """
        SELECT room_type, MIN(price) AS min_price, MAX(price) AS max_price
        FROM hotel_rooms
        GROUP BY room_type
        """
        self.cur.execute(sql_min_max_prices)
        self.min_max_prices = {'room_type': [], 'min_price': [], 'max_price': []}
        
        for row in self.cur:
            self.min_max_prices['room_type'].append(row['room_type'])
            self.min_max_prices['min_price'].append(float(row['min_price']))
            self.min_max_prices['max_price'].append(float(row['max_price']))
        ...
        print("Min/Max Prices Data:", self.min_max_prices)
