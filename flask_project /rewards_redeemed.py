...
class rewards_redeemed(baseObject):
    ...
    def getRewardStats(self):
        ...
        # Points Distribution by Reward Code
        sql_points_distribution = """
        SELECT reward_code, AVG(points_used) AS avg_points, MIN(points_used) AS min_points, MAX(points_used) AS max_points
        FROM hotel_rewards_redeemed
        GROUP BY reward_code
        ORDER BY avg_points DESC
        """
        self.cur.execute(sql_points_distribution)
        self.points_distribution = {'reward_code': [], 'avg_points': [], 'min_points': [], 'max_points': []}

        for row in self.cur:
            self.points_distribution['reward_code'].append(row['reward_code'])
            self.points_distribution['avg_points'].append(float(row['avg_points']))
            self.points_distribution['min_points'].append(float(row['min_points']))
            self.points_distribution['max_points'].append(float(row['max_points']))
        ...
        print("Points Distribution Data:", self.points_distribution)
